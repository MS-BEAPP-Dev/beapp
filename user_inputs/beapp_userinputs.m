...
grp_proc_info.beapp_itpc_ersp_params.win_size= 0.128; %CURRENTLY NOT USED; the win_size (in seconds) to calculate ERSP and ITPC from the ERPs of the composed dataset (e.g. should result in a number of samples an integer and divide trials equaly ex: 10)
grp_proc_info.beapp_itpc_ersp_params.baseline_norm = 1;
grp_proc_info.beapp_itpc_ersp_params.set_freq_range = 1; %default 0 = off [minfreq 50], minfreq being determined by the number of data points, cycles, etc
grp_proc_info.beapp_itpc_ersp_params.min_freq = 2;  % def = 2; minimum frequency limit
grp_proc_info.beapp_itpc_ersp_params.max_freq = 50;  % def = 50; maximum frequency limit
grp_proc_info.beapp_itpc_ersp_params.min_cyc = 2; % def = 2; number of cycles in each Morlet wavelet
grp_proc_info.beapp_itpc_ersp_params.max_cyc = 2; % def = 0; 0 = use same window size across freqs, 1 = use same number of cycles, 0<max_cyc<1 = increase cycles  linearly, max_cyc>1 = incr cycles from min_cyc up to max cyc
